...
// ─── Main MockTest ─────────────────────────────────────────────────────────────
export default function MockTestNew() {
  ...
  // Track time taken for result screen
  const timeTakenRef = useRef(0);

  // Lock questions in a ref so timer dependency doesn't reshuffle
  ...
  useEffect(() => {
    ...
    if (timeLeft <= 0) {
      timeTakenRef.current = TOTAL_TIME;
      setFinished(true);
      setShowResult(true);
      return;
    }
  ...
  const handleConfirmSubmit = () => {
    timeTakenRef.current = TOTAL_TIME - timeLeft;
    setFinished(true);
